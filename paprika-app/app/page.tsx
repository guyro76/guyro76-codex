...
export default function Home() {
  ...
  async function loadCloud(userId: string) {
    ...
    setClients(clientResult.data || []); setTasks(taskResult.data || []);
  }

  function saveLocal(nextClients = clients, nextTasks = tasks) {
    if (!supabase) { localStorage.setItem('paprika-clients', JSON.stringify(nextClients)); localStorage.setItem('paprika-tasks', JSON.stringify(nextTasks)); }
  }
  ...
  async function addClient() {
    ...
    const record = { id: crypto.randomUUID(), user_id: user?.id, name: newClient.name.trim(), industry: newClient.industry.trim(), platform: newClient.platform, monthly_budget: Number(newClient.budget) || 0, roas: 0, health: 75 };
    if (supabase && user) { const { data, error } = await supabase.from('clients').insert(record).select().single(); if (error) return setNotice(error.message); setClients(value => [...value, data]); }
    else { const next = [...clients, record]; setClients(next); saveLocal(next, tasks); }
  ...
  async function addTask() {
    ...
    const record = { id: crypto.randomUUID(), user_id: user?.id, client_id: null, title: newTask.title.trim(), client_name: newTask.client_name || clients[0]?.name || 'ללא לקוח', due_label: newTask.due_label, priority: newTask.priority, status: 'לביצוע' as TaskStatus };
    if (supabase && user) { const { data, error } = await supabase.from('agency_tasks').insert(record).select().single(); if (error) return setNotice(error.message); setTasks(value => [...value, data]); }
    else { const next = [...tasks, record]; setTasks(next); saveLocal(clients, next); }
  ...
  async function cycleTask(task: AgencyTask) {
    ...
    const status = statuses[task.status]; if (supabase && user) await supabase.from('agency_tasks').update({ status }).eq('id', task.id);
    const next = tasks.map(item => item.id === task.id ? { ...item, status } : item); setTasks(next); saveLocal(clients, next);
  }

  const metrics = { budget: clients.reduce((sum, item) => sum + item.monthly_budget, 0), roas: clients.length ? clients.reduce((sum, item) => sum + item.roas, 0) / clients.length : 0, open: tasks.filter(item => item.status !== 'הושלם').length, risk: clients.filter(item => item.health < 70).length };
  ...
  if (supabase && supabaseConfigured && !user) return <main className="login card"><div className="brand"><div className="logo"/><div><h1>פפריקה</h1><small>PPC AGENCY OS</small></div></div><h2>כניסה מאובטחת</h2><p>הזן מייל לקבלת קישור כניסה.</p><input className="input" value={email} onChange={event => setEmail(event.target.value)}/><button className="btn primary" onClick={signIn}><LogIn size={17}/>שליחת קישור</button>{notice && <div className="notice">{notice}</div>}</main>;

  return <div className="shell"><aside className={`side ${menu ? 'open' : ''}`}><div className="brand"><div className="logo"/><div><h1>פפריקה</h1><small>PPC AGENCY OS</small></div></div><nav className="nav">{navigation.map(([id,label,Icon]) => <button key={id} className={page === id ? 'active' : ''} onClick={() => { setPage(id); setMenu(false); }}><Icon size={19}/>{label}</button>)}</nav></aside><main className="main"><header className="top"><div className="top-title"><button className="btn secondary mobile" onClick={() => setMenu(true)}><Menu size={18}/></button><div><small>מערכת ניהול סוכנות</small><h2>{navigation.find(item => item[0] === page)?.[1]}</h2></div></div><div className="top-actions"><button className="btn secondary" onClick={() => setTaskModal(true)}><Plus size={17}/>משימה</button>{user && <button className="btn secondary" onClick={() => supabase?.auth.signOut()}><LogOut size={17}/>יציאה</button>}</div></header><div className="content">
    ...
    {page === 'dashboard' && <><section className="hero"><h3>בוקר טוב גיא, יש {metrics.open} פעולות פתוחות</h3><p>חדר מצב לניהול סוכנות Google Ads ו-Meta Ads.</p></section><section className="grid metrics"><Metric icon={CircleDollarSign} label="תקציב" value={money(metrics.budget)}/><Metric icon={BarChart3} label="ROAS" value={metrics.roas.toFixed(2)}/><Metric icon={ListTodo} label="משימות" value={String(metrics.open)}/><Metric icon={AlertTriangle} label="בסיכון" value={String(metrics.risk)}/></section><section className="grid two"><div className="card"><div className="toolbar"><h3>משימות דחופות</h3><button className="btn primary" onClick={() => setTaskModal(true)}><Plus size={16}/>חדשה</button></div><div className="list">{tasks.filter(item => item.status !== 'הושלם').slice(0,5).map(item => <TaskRow key={item.id} task={item} cycle={cycleTask}/>)}</div></div><div className="card tips"><span className="pill"><Lightbulb size={14}/> טיפ יומי</span><h3>{tips[tipIndex][0]}</h3><p>{tips[tipIndex][1]}</p><button className="btn primary" onClick={() => setTipIndex((tipIndex + 1) % tips.length)}><RefreshCcw size={16}/>טיפ נוסף</button></div></section></>}
    {page === 'campaigns' && <CampaignLab/>}
    {page === 'clients' && <><section className="hero"><h3>לקוחות 360°</h3><p>תיקי לקוחות, תקציבים ובריאות.</p></section><div className="toolbar"><input className="input search" placeholder="חיפוש" value={search} onChange={event => setSearch(event.target.value)}/><button className="btn primary" onClick={() => setClientModal(true)}><Plus size={17}/>לקוח חדש</button></div><div className="card table"><table><thead><tr><th>לקוח</th><th>תחום</th><th>פלטפורמה</th><th>תקציב</th><th>ROAS</th><th>בריאות</th><th/></tr></thead><tbody>{filteredClients.map(item => <tr key={item.id}><td><b>{item.name}</b></td><td>{item.industry}</td><td>{item.platform}</td><td>{money(item.monthly_budget)}</td><td>{item.roas}</td><td>{item.health}/100</td><td><button className="btn secondary" onClick={() => { if (confirm('למחוק את הלקוח?')) setClients(clients.filter(client => client.id !== item.id)); }}><Trash2 size={15}/></button></td></tr>)}</tbody></table></div></>}
    {page === 'tasks' && <><section className="hero"><h3>משימות ו-SLA</h3><p>לחיצה על סטטוס מקדמת את המשימה.</p></section><div className="grid task-columns">{(['לביצוע','בטיפול','הושלם'] as TaskStatus[]).map(status => <div className="card" key={status}><h3>{status}</h3><div className="list">{tasks.filter(item => item.status === status).map(item => <TaskRow key={item.id} task={item} cycle={cycleTask}/>)}</div></div>)}</div></>}
    {page === 'knowledge' && <><section className="hero"><h3>טיפים ומילון PPC</h3><p>ידע מעשי ל-Google Ads ול-Meta Ads.</p></section><div className="toolbar"><input className="input search" value={search} onChange={event => setSearch(event.target.value)} placeholder="חיפוש מושג"/><select className="input" value={provider} onChange={event => setProvider(event.target.value as typeof provider)}><option>הכל</option><option>Google Ads</option><option>Meta Ads</option></select></div><div className="grid glossary-grid">{filteredGlossary.map(item => <article className="card" key={item.term}><span className="pill">{item.provider}</span><h3 dir="ltr">{item.term}</h3><p>{item.definition}</p><div className="practical"><b>בפועל</b><br/>{item.practical}</div></article>)}</div></>}
    {page === 'reports' && <><section className="hero"><h3>דוחות</h3><p>סיכום ביצועים ופעולות.</p></section><div className="card"><h3>סיכום סוכנות</h3><p>לקוחות: {clients.length}</p><p>תקציב: {money(metrics.budget)}</p><p>משימות פתוחות: {metrics.open}</p><button className="btn primary" onClick={() => window.print()}><FileText size={17}/>PDF</button></div></>}
    {page === 'qa' && <><section className="hero"><h3>QA ובקרה</h3><p>בדיקת תשתית ומנוע הניתוח.</p></section><button className="btn primary" onClick={() => setQa([clients.length ? '✓ לקוחות נטענו' : '✗ אין לקוחות', tasks.every(item => item.title) ? '✓ משימות תקינות' : '✗ משימה חסרה', glossary.length >= 10 ? '✓ מילון נטען' : '✗ מילון חסר', '✓ מנוע CSV זמין', supabaseConfigured ? '✓ Supabase מוגדר' : '• Supabase טרם הוגדר'])}><Activity size={17}/>הרץ QA</button><div className="card list">{qa.map(item => <div className="row" key={item}>{item}</div>)}</div></>}
    {page === 'settings' && <><section className="hero"><h3>חיבורים והגדרות</h3><p>Google Ads, Meta Ads, Supabase ותוסף הדפדפן.</p></section><div className="grid two"><div className="card"><h3><Database size={19}/> Supabase</h3><p>{supabaseConfigured ? 'משתני הסביבה קיימים.' : 'נדרש להגדיר משתני סביבה ולהריץ מיגרציות.'}</p></div><div className="card"><h3><Zap size={19}/> Google Ads</h3><p>חיבור קריאה בלבד באמצעות OAuth ו-Developer Token.</p><a className="btn primary" href="/api/google/oauth/start">חיבור Google Ads</a></div><div className="card"><h3><Zap size={19}/> Meta Ads</h3><p>חיבור קריאה בלבד באמצעות ads_read.</p><span className="pill">מוכן להגדרת App</span></div><div className="card"><h3><ScanSearch size={19}/> תוסף דפדפן</h3><p>Side Panel לניתוח המסך הפתוח והעברת Snapshot לפפריקה.</p><span className="pill">בפיתוח</span></div></div></>}
  </div></main></div>
...
function Metric({ icon: Icon, label, value }: { icon: typeof Activity; label: string; value: string }) { return <div className="card"><Icon size={20}/><small>{label}</small><h3>{value}</h3></div>; }
...
function Modal({ title, close, children }: { title: string; close: () => void; children: React.ReactNode }) { return <div className="modal-backdrop"><div className="modal"><div className="toolbar"><h3>{title}</h3><button className="btn secondary" onClick={close}><X size={17}/></button></div><div className="list">{children}</div></div></div>; }
