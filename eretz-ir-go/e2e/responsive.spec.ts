...
const SIZES = [
  ...
  /**
   * טלפון שכוב. בודק בחנות מסובב את המכשיר — זו אחת הפעולות
   * הראשונות שהוא עושה — והמניפסט מצהיר `configChanges` הכולל
   * `orientation`, כלומר האפליקציה **לא** נטענת מחדש אלא צריכה
   * להסתדר עם הגובה החדש. 360 גובה הוא בדיוק המצב שבו מקלדת
   * וכפתורים נדחסים.
   */
  { name: 'טלפון שכוב', width: 740, height: 360 },
