...
test('✍️ תשובה שנפסלה על כתיב מתקבלת בלחיצה, והניקוד עולה', async ({ page }) => {
  ...
  /**
   * חילוץ המילה שהוצעה — **בלי להסיר רווחים**.
   *
   * הגרסה הקודמת ניקתה כל תו שאינו אות עברית, כולל הרווח, ואז
   * הרכיבה מחדש את הקידומת. זה עבד לכל מילה אחת ונשבר על תשובה
   * משתי מילים: "חוף השנהב" הפך ל-"חוףהשנהב", ואז החיפוש בשורת
   * התשובה לא מצא כלום — בזמן שהמשחק התנהג נכון לגמרי.
   */
  const suggested = ((await accept.innerText()).match(/התכוונתי\s*ל\s*(.+?)\s*[?!]?$/s)?.[1] ?? '')
    .replace(/["'׳״]/g, '')
    .trim();
  expect(suggested, 'לא הצלחנו לחלץ את המילה שהוצעה').not.toBe('');
