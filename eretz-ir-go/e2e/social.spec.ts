...
test('🎯 לחיצה על מצב משחק מתקדמת מיד למסך הבא', async ({ page }) => {
  // דווח פעמיים: "לוחץ על משחק יחיד וזה לא מתקדם". קלף שנראה כמו
  // כפתור ולא מוביל לשום מקום נקרא כשבור.
...
test('🎴 קלף מסתורי מדלג ישר להגרלת האות', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('./');
  await page.getByRole('button', { name: /בואו נשחק/ }).click();
  await page.getByRole('button', { name: /משחק חדש/ }).click();
  ...
  await expect(page.locator('.letter-wheel')).toBeVisible({ timeout: 10_000 });
});
