...
/**
 * עמוד מחיקת חשבון פומבי.
 *
 * Google Play דורשת, מרגע שאפשר לפתוח חשבון באפליקציה, **כתובת
 * אינטרנט פתוחה** שבה אפשר לבקש מחיקה של החשבון והמידע שלו — בלי
 * להתקין את האפליקציה ובלי להיכנס אליה. הכפתור שבתוך המשחק אינו
 * מספיק לדרישה הזו: מי שאיבד את המכשיר או שכח את הסיסמה לא יכול
 * להגיע אליו.
 */
describe('עמוד מחיקת חשבון', () => {
  const page = readFileSync(resolve(root, 'public/delete-account.html'), 'utf8');
  ...
  /** שני המסלולים: מתוך המשחק, ובלי גישה אליו */
  it('מסביר את שתי דרכי המחיקה', () => {
    expect(page).toContain('מחיקת החשבון');
    expect(page).toContain('guyro76@gmail.com');
  });

  /**
   * הדרישה אינה "עמוד שכתוב בו מחיקה" אלא גילוי מלא: מה נמחק, מה
   * נשמר בכל זאת, וכמה זמן זה לוקח.
   */
  it('מפרט מה נמחק, מה נשמר וכמה זמן', () => {
    expect(page).toContain('מה בדיוק נמחק');
    expect(page).toContain('מה לא נמחק');
    expect(page).toContain('30 יום');
  });

  it('מקושר ממדיניות הפרטיות', () => {
    expect(policy).toContain('/delete-account.html');
  });

  /**
   * הורה חייב להיות מסוגל לבקש מחיקה של חשבון ילד גם כשהוא עצמו
   * לא פתח אותו. זו דרישת Families ולא נימוס.
   */
  it('מאפשר להורה לבקש מחיקה של חשבון ילד', () => {
    expect(page).toContain('הורה');
  });
});


/**
 * מסך ההרשמה — מה מותר ומה אסור לומר בו.
 *
 * המשחק מיועד לילדים, אבל **החשבון הוא של ההורה**. חוק הגנת
 * הפרטיות (תיקון 13) ו-COPPA האמריקאי מצפים שמסך ההרשמה לא יזמין
 * ילד לפתוח חשבון בעצמו, וזה המקום היחיד שבו אפשר לומר את זה לפני
 * שנפתח חשבון.
 */
describe('מסך ההרשמה', () => {
  const login = readFileSync(resolve(root, 'src/screens/Login.tsx'), 'utf8');
  const visible = login
    .split('\n')
    .filter((l) => !l.trimStart().startsWith('*') && !l.trimStart().startsWith('//'))
    .join('\n');

  it('אומר שהחשבון של ההורה', () => {
    expect(visible).toContain('להורה');
  });

  it('מבהיר שאפשר לשחק בלי חשבון בכלל', () => {
    expect(visible).toContain('בלי הרשמה');
  });

  it('מקשר גם לפרטיות וגם לתנאי השימוש', () => {
    expect(visible).toContain('/privacy.html');
    expect(visible).toContain('/terms.html');
  });

  /**
   * הבאג שנתפס כאן: המסך הבטיח "אנחנו לא מציגים פרסומות", וזה הפסיק
   * להיות נכון ברגע שנוספה גרסה חינמית עם מודעות. הצהרה שגויה במסך
   * הרשמה אינה שגיאת ניסוח — היא מצג שווא.
   */
  it('לא מבטיח שאין פרסומות', () => {
    expect(visible).not.toContain('לא מציגים פרסומות');
    expect(visible).not.toMatch(/אין\s+פרסומות\s*[.,]/);
  });

  it('אומר את האמת על הפרסומות בשתי הגרסאות', () => {
    expect(visible).toContain('שאינן מותאמות אישית');
