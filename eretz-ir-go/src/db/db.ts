...
export interface ImageCacheRow {
  ...
  licenseUrl?: string;
  /**
   * שם האתר שממנו הגיעה התמונה, לתצוגה בשורת הקרדיט.
   *
   * נולד כשנוסף מקור שני (Openverse): עד אז כל התמונות היו
   * מוויקיפדיה, והשם נכתב קבוע בקוד. תמונה מ-Flickr שמוצגת עם
   * הכיתוב "ויקיפדיה העברית" היא ייחוס שגוי, וזו בדיוק ההפרה
   * שהקרדיט אמור למנוע. שדה לא מאונדקס — אין צורך בגרסה חדשה.
   */
  source?: string;
