    ...
    <div className="screen center">
      ...
      {mode === 'choose' ? (
        ...
        <div className="card" style={{ maxWidth: 400, margin: '0 auto', display: 'grid', gap: 10 }}>
          ...
          {/*
            נאמר במפורש בתוך טופס ההרשמה ולא רק בשורה הקטנה בתחתית
            המסך: ילד שמגיע לכאן לבד צריך לדעת שזה לא המסך שלו.
          */}
          {mode === 'email-up' && (
            <p className="dim" style={{ margin: 0, fontSize: '0.85rem' }}>
              👋 המסך הזה מיועד להורה. הילד לא צריך חשבון כדי לשחק — המשחק עובד גם בלי הרשמה בכלל.
            </p>
          )}
      ...
      {/*
        שלוש אמירות, וכל אחת מהן נדרשת:

        1. **החשבון הוא של מבוגר.** המשחק מיועד לילדים, אבל מי שפותח
           חשבון הוא ההורה. חוק הגנת הפרטיות (תיקון 13) ו-COPPA
           מצפים שהמסך לא יזמין ילד לפתוח חשבון בעצמו, וזה המקום
           היחיד שבו אפשר לומר את זה לפני שנפתח חשבון.
        2. **קישור לשני המסמכים**, ולא רק לפרטיות.
        3. **אמירה נכונה על פרסומות.** קודם היה כתוב כאן "אנחנו לא
           מציגים פרסומות" — וזה הפסיק להיות נכון ברגע שנוספה גרסה
           חינמית עם מודעות. הצהרה שגויה במסך הרשמה היא בדיוק מה
           שאסור שיהיה.
      */}
      <p className="dim" style={{ fontSize: '0.82rem', maxWidth: 400, margin: '18px auto 0' }}>
        החשבון נועד להורה או למבוגר אחראי. בכניסה אתם מאשרים את{' '}
        ...
        . אנחנו לא אוספים מידע על הילד, לא עוקבים אחריו ולא מוכרים מידע לאף אחד. בגרסה החינמית מוצגות
        מודעות שאינן מותאמות אישית; בגרסה בתשלום אין מודעות כלל.
      </p>
    </div>
  );
}
