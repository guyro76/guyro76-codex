...
export function validateAnswer(params: {
  ...
  /**
   * מילון אישי — **לפני** פסילת הקטגוריה, ולא אחריה.
   *
   * ## הבאג שזה מתקן
   *
   * הבדיקה הזו ישבה מתחת לבלוק "נמצא במאגר אבל בקטגוריה אחרת",
   * כלומר אחרי `return` שפוסל. התוצאה: מילה שההורה אישר במפורש
   * נדחתה בכל זאת — **בתנאי שהיא קיימת במאגר תחת קטגוריה אחרת**.
   *
   * זה בדיוק המקרה שבו הפיצ'ר נועד לעזור. ילד כותב "ורד" בקטגוריית
   * חי, המשחק פוסל, ההורה מאשר, ובסיבוב הבא — נפסל שוב. שסתום
   * הביטחון האנושי פשוט לא עשה כלום, בשקט.
   *
   * ## למה הסדר הזה נכון
   *
   * המאגר מתועד כ**רשימת זרעים ולא מילון**: היעדר קטגוריה בו אינו
   * הוכחה שהמילה אינה שייכת אליה. אישור מפורש של הורה הוא עדות
   * חזקה יותר מהיעדר רשומה, ולכן הוא גובר.
   *
   * מה שנשאר **מעל** הבדיקה הזו הוא מה שאישור הורה אינו יכול
   * לעקוף: תשובה כפולה באותו סיבוב, אות שגויה, וכללי הקטגוריה
   * (מילה אחת, עברית בלבד). אלה עובדות ולא דעות.
   */
